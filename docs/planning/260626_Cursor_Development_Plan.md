...
### Node usage = a CONTRIBUTION CREDIT, not a cost (implemented meter 260626; minting = next)
Implemented 260626: `POST /api/node-ops/usage` → `cost_events` rows with `source: 'merlin-node'`, `billedToTenantKey: true`, `costCents: 0`. Merlin emits one per locally-served turn (gateway `/api/ai` = rich token counts; agent path = provider/latency). These rows are NOT a platform bill — they are **proof-of-contribution**: a Merlin spending its OWN compute/quota to serve intelligence to the commons.

**Economic semantics (the intent behind the meter):** every `merlin-node` usage row should **CREDIT the node owner** in karma coins / Angel tokens (the inverse of a debit). This is the "meter was already running" capstone — the receipt of shared mind becomes value flowing *back to the contributor*.
- **Minting source:** the `cost_events` ledger (server-side, trustless — Core only credits what it witnessed reported against a resolved tenant/node).
- **Weighting (physics-based, later):** local-GPU turns weigh more than proxied-cloud turns (real electricity vs shared quota); credit ∝ verifiable work (tokens × time, with a local-vs-cloud multiplier). Tiny per-turn values that only matter at scale — no one priced out, mobile micro-contributions count.
- **Settlement:** credits land in Core's existing **wallet / karma** surface; **eventually convertible** into other currency types (Stripe payout / compute credits / premium-model access). Karma-first so it *feels* like participation, not crypto.
- **Anti-fragile:** routing + metering work today with the economy at zero; minting is a pure incentive layer bolted on the ledger, never a dependency. **TODO:** Core aggregator over `cost_events WHERE source='merlin-node'` → per-owner credit balance → mint into wallet/karma; expose a "Contributions" view alongside AI Costs (costs = what you spent; contributions = what you earned back).
- **Attribution is already sufficient (no backfill needed):** shipped rows carry `tenantId` + `metadata.nodeId`. Each registered node has a stable `nodeUserId` (its system-user, provisioned in `provisionNodeIdentity`) and an endeavor binding whose members include the human owner — so minting can resolve `nodeId → nodeUserId → owner` from the existing node registry. Optional polish later: also stamp `userId` (resolved owner) on the usage row at ingest to make the aggregation a single-pass group-by.
