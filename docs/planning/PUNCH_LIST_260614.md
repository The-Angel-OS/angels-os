...
- ✅ **Engine SHIPPED + VERIFIED (4f8ee8a + b5bccf1)** — plan create/list 200; checkout 409s on a non-Connect tenant (proves dues need the bank); Memberships read auth-gated. ⚠️ FOUND+FIXED a CRITICAL latent bug: the `settings` table was MISSING on BOTH prod DBs → SettingService 500'd → feature flags + election persistence (#131) + membership plans all broken; provisioned settings table + ensure-settings-table endpoint. — recurring revenue for every vertical (church pledges, gym memberships, makerspace/Toastmasters dues, market booth fees) + our platform fee. Memberships collection (schema-first, table on both prod DBs; "who's a member/dues current" primitive); plans in settings bag (/membership-ops/plans); /membership-ops/checkout = Stripe Checkout subscription as a Connect DESTINATION charge (dues→endeavor) + application_fee_percent (default 2% = our revenue); stripe-webhooks customer.subscription.* → upsert Memberships. Host-resolved tenant; 409s if endeavor not Connect-onboarded.
- ✅ **Member-facing surface SHIPPED (2f73f17, READY/live)** — Membership/Join BLOCK (src/blocks/Membership; runtime-fetches the host endeavor's active plans → name/email → Stripe Connect subscription checkout; registered in Pages + RenderBlocks); `create_membership_plan` LEO tool (admin-gated, upsertMembershipPlan, factory-before-button); wired into the church template (optional membershipPlans → Membership page; default parishes stay donation-based) + a NEW generic **fitness/gym template** (provisionFitnessSite + /provision-ops/fitness-template — CrossFit/yoga/Pilates/martial-arts; Home/Classes/Pricing(Join)/Coaches/Get Started/Contact + default plans; gyms=commercial→kendev.co). [[project_membership_surface]]
- ✅ **Member dashboard SHIPPED (9901525)** — GET /membership-ops/my (signed-in member's membership: plan/status/renewal, by user id or email) + POST /membership-ops/portal (Stripe Billing Portal: update card/invoices/cancel; portal on PLATFORM account since destination-charge subs live there); Membership block is now member-aware (active member → "Your membership" card + Manage button instead of plan picker). Webhook already syncs sub created/updated/deleted incl canceled.
...
- ⬜ **"Rent our space" facility-rental block/page** (church template) — bookable rooms/hall = booking engine + deposit + AgreementForm (all shipped); second revenue line for congregations → own Connect account. Friendly on-ramp that funds the Sanctuary Parking mission (docs/strategy/SAFE_PARKING_VERTICAL.md).
- 🅿️ **Sanctuary Parking vertical** — Guardian Angel use of booking + Signatures + Presence/Quests + Karma + federation trust to coordinate church-lot safe overnight parking; portable cross-host good standing = the differentiator. Needs policy carrot (liability shield/permit/stipend) for the non-software blockers. Strategy + LinkedIn article: docs/strategy/SAFE_PARKING_VERTICAL.md. ⚠️ St Alfred's reference = consent-gated.
...
## 7b. ⚠️ Dashboard authorization (SECURITY — found + FIXED + VERIFIED 2026-06-14)  ([[project_auth_context_refactor]])
- ✅ **CRITICAL fix shipped + verified live** — `/dashboard/admin/*` + `/dashboard/orders` were PUBLICLY accessible (auth-optional layout + no middleware gate + overrideAccess queries); UNAUTH could read a tenant's financials/PII by direct URL. Now: middleware (60166bb) edge-redirects anon (no payload-token) subtree-wide → verified 307→/dashboard, no data on follow; per-page requirePortalManager (e2a6310) on all admin pages + /orders enforces role (platform-admin/tenant-admin/manager); 7-test regression lock (f260315). Verified: anon blocked, malformed cookie = error-boundary no-data, non-admin pages not over-blocked.
- ⚠️ **LESSON:** layout-level `redirect()` does NOT reliably gate child pages under Next 16 (two false starts). Gate dashboard auth at MIDDLEWARE (anon, cookie-presence) + PAGE (role, requirePortalManager + `export const dynamic='force-dynamic'`) — NOT layouts. And verify by rendered component/DATA or a 3xx, never a label-string grep (one false-positive this session).
- ✅ **Content-page audit DONE (aa27e2e)** — 10 content/management pages (appointments[PII], posts, products, pages, media, quests, events, projects, services, availability) were server-rendered with overrideAccess, ungated, OUTSIDE /dashboard/admin (middleware didn't cover them). All now gated with requirePortalManager() + force-dynamic. ⚠️ NOTE: gate is the MANAGER bar (platform-admin or tenant_admin/manager) — slightly stricter than nav's isBusinessOwner (any non-customer); fine today (no non-admin staff roles exist), revisit if a staff role is added.
- ⬜ **Stricter platform-only gate** — provision/tenants/federation admin pages could require adminPortal (platform-admin only) on top of the manager bar.
- ✅ **Space deletion fixed (f3b7e6a)** — deleting a space 500'd ("Something went wrong") on the not-null space_id of child rows (memberships/messages/channels are SET NULL but NOT NULL); added a Spaces beforeDelete cascade hook (memberships→messages→channels). Unrelated to auth but same "looks broken after refactor" QA pass.
