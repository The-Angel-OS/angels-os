 ...
 * once on the first paid transition, never double-decrement on a webhook retry,
...
describe('markOrderPaidAndDecrementInventory', () => {
  it('decrements each tracked product by its quantity, then marks the order paid', async () => {
    ...
    expect(payload.updates.find((u: any) => u.collection === 'orders')?.data.status).toBe('paid')
  })
  ...
  it('is idempotent — an already-paid order does nothing (no double-decrement)', async () => {
    const payload = makePayload({ status: 'paid', items: [{ product: { id: 10, inventory: 50 }, quantity: 1 }] })
    const res = await markOrderPaidAndDecrementInventory(payload, 1)
  ...
  it('skips untracked (unlimited/digital) products', async () => {
    ...
    // order still marked paid, but no product update
