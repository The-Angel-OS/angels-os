...
export const federationSuitcaseImportHandler: PayloadHandler = async (req) => {
  // Auth check — require an authenticated admin/super_admin user
  const user = (req as any).user
  if (!user) {
    return Response.json({ error: 'Authentication required for suitcase import' }, { status: 401 })
  }
  const userRoles: string[] = Array.isArray(user.roles) ? user.roles : []
  if (!userRoles.includes('super_admin') && !userRoles.includes('admin')) {
    return Response.json({ error: 'Admin role required for suitcase import' }, { status: 403 })
  }
