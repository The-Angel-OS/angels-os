 ...
 *
 * Server component that shows tenant-level invitation details and an accept button.
 * Handles expired, already-accepted, and invalid tokens gracefully.
 */
export default async function TenantInviteLandingPage({
  params,
}: {
  ...
  const enterpriseName = tenant?.branding?.siteName || tenant?.name || 'Unknown Enterprise'
  ...
  const role = (membership.role || 'tenant_member').replace('tenant_', '').replace('_', ' ')
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        <div className="w-full max-w-md rounded-xl border border-border bg-card p-8 text-center shadow-lg">
          ...
          <p className="mb-6 text-muted-foreground">
            You&apos;ve already joined <strong>{enterpriseName}</strong>.
          </p>
          <Link
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-background p-4">
        <div className="w-full max-w-md rounded-xl border border-border bg-card p-8 text-center shadow-lg">
          ...
          <p className="mb-6 text-muted-foreground">
            This invitation to <strong>{enterpriseName}</strong> has expired.
            Ask {inviterName} to send a new one.
          </p>
          <Link href={`/${locale}`} className="text-primary hover:underline">
            Go to Homepage
          </Link>
