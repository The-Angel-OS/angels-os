...
/**
 * Create GitHub Issues from Angel OS MVP Roadmap
 * Updated to match C:\Dev\angels-os\docs\GITHUB_ISSUES_MVP.md exactly
 * 
 * Usage:
 * 1. Install GitHub CLI: gh auth login
 * 2. Run: node scripts/create-github-issues.mjs
 * 
 * Or manually copy-paste the formatted issues from console output.
 */
...
`
  ...
  // Continue with remaining 12 issues (9-21)...
]

// Add remaining issues (9-21)
...
**Epic:** Tenant Provisioning & Onboarding
**Priority:** Critical
`
  },
...
**Epic:** AI Bus & Guardian Communication
**Priority:** High
`
  },
...
**Epic:** Federation & Enterprise System
**Priority:** Medium
`
  },
...
// Combine all issues
const allIssues = [...issues, ...remainingIssues]
...
function createIssue(issue) {
  ...
  try {
    const command = `gh issue create --repo ${REPO} --title "${issue.title}" --body "${issue.body.replace(/"/g, '\\"')}" --label ${labelsArg}`
...
function main() {
  console.log('🦅 Angel OS MVP - GitHub Issues Creation')
  console.log(`Repository: ${REPO}`)
  console.log(`Dry Run: ${DRY_RUN}`)
  console.log(`Total Issues: ${allIssues.length}\\n`)
  
  if (DRY_RUN) {
    console.log('\\n📋 DRY RUN MODE - Showing formatted issues:\\n')
    allIssues.forEach((issue, index) => {
      console.log(`\\n[${index + 1}/${allIssues.length}] Creating: ${issue.title}`)
      createIssue(issue)
    })
    
    console.log(`\\n🚀 To create issues for real:`)
    console.log(`1. Install GitHub CLI: gh auth login`)
    console.log(`2. Edit this script: Set DRY_RUN = false`)
    console.log(`3. Run: node scripts/create-github-issues.mjs`)
